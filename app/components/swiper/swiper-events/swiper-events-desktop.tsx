...
export default function Swiper_Events_Desktop({ events, title = "", odd = false, className = "" }: { events: EventType[], title?:string, odd?: boolean, className?:string }) {
  ...
  return (
    <Swiper className={`${styles['namespace-container']} swiper-responsive swiper-responsive-desktop ${className}`} modules={[Autoplay]} autoplay={{ delay: sliderDelay, disableOnInteraction: false }} loop={events.length > 2} slidesPerView={2}
