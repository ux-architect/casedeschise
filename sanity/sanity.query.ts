  ...
  async (eventType: string): Promise<SignupFormType> => {
    return client.fetch(
      groq`*[_type == $eventType][0]{
        ...
        s1_title,
        s1_subtitle,
        s1_projects[]{ "image": image.asset->url, name, subtitle, code, info, address, soldOut },
        s1_optionalItems{ texts[]{ text }, checkboxes[]{ checkboxLabel, checkboxCode, infoText } },
        s2_title,
        s2_subtitle,
        s2_projects[]{ "image": image.asset->url, name, subtitle, code, info, address, soldOut },
        s2_optionalItems{ texts[]{ text }, checkboxes[]{ checkboxLabel, checkboxCode, infoText } },
        s3_title,
        s3_subtitle,
        s3_projects[]{ "image": image.asset->url, name, subtitle, code, info, address, soldOut },
        s3_optionalItems{ texts[]{ text }, checkboxes[]{ checkboxLabel, checkboxCode, infoText } },
        email_template,
      }`,
      { eventType },
    );
